...
out_pref='out'
out_fn=out_pref+'_fittedCt_HH.dat'
in_file_list = [out_pref+'_CtintHH.dat']
bNoFast=False #True #False
nc=-1
...
for i in range(num_vecs):
    ...
    if num_comp == -1:
        ...
        if bUseSFast:
            chi, names, pars, errs, ymodel = findbest_Expstyle_fits(dt[i], Ct[i], dy_loc, par_list=[2,3,5,7,9], threshold=1.0)
        else:
            chi, names, pars, errs, ymodel = findbest_Expstyle_fits(dt[i], Ct[i], dy_loc, par_list=[2,4,6,8], threshold=1.0)
        num_pars=len(names)
    else:
