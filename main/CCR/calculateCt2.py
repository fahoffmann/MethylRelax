...
Xseltxt='(name HE2 and resname MET) or (name HG22 and resname ILE) or (name HD12 and resname ILE) or (name HG12 and resname VAL) or (name HG22 and resname VAL) or (name HD12 and resname LEU) or (name HD22 and resname LEU) or (name HG22 and resname THR) or (name HB2 and resname ALA)'
Hseltxt2='(name HE3 and resname MET) or (name HG23 and resname ILE) or (name HD13 and resname ILE) or (name HG13 and resname VAL) or (name HG23 and resname VAL) or (name HD13 and resname LEU) or (name HD23 and resname LEU) or (name HG23 and resname THR) or (name HB3 and resname ALA)'
...
if bDoCt:
        ...
        print_sxylist(out_pref+'_Ctext_HH.dat', resXH, dt, np.stack( (Ct.T,dCt.T), axis=-1) )
        print_sxylist(out_pref+'_Ctext_HHH.dat', resCX, dt, np.stack( (Ct_C.T,dCt_C.T), axis=-1) )
        ...
        print_sxylist(out_pref+'_CtintHH.dat', resXH, dt, np.stack( (Ct.T,dCt.T), axis=-1) )
        print_sxylist(out_pref+'_Ctint_HHH.dat', resCX, dt, np.stack( (Ct_C.T,dCt_C.T), axis=-1) )
        del Ct, dCt
        del Ct_C, dCt_C
sh = vecXHfit.shape
...
if bDoVecDistrib:
    ...
    if not bDoVecHist:
        ...
        for i in range(nBonds):
            ...
            hist_list[i]=hist
        if bBinary:
            np.savez_compressed(out_pref+'_vecHistogramHH.npz', names=resXH, dataType='LambertCylindrical', bHistogram=True, edges=edgesAll, axisLabels=['phi','cos(theta)'], data=hist_list)
            np.savez_compressed(out_pref+'_vecHistogram_HHH.npz', names=resCX, dataType='LambertCylindrical', bHistogram=True, edges=edgesAll, axisLabels=['phi','cos(theta)'], data=hist_list)
        else:
            for i in range(nBonds):
