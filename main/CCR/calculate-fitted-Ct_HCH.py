...
def func_exp_decay6(t, A, tau_a, B, tau_b, G, tau_g ):
    return (-0.316-A-B-G) + A*np.exp(-t/tau_a) + B*np.exp(-t/tau_b) + G*np.exp(-t/tau_g)
...
def func_exp_decay8(t, A, tau_a, B, tau_b, G, tau_g, D, tau_d):
    return (-0.316-A-B-G-D) + A*np.exp(-t/tau_a) + B*np.exp(-t/tau_b) + G*np.exp(-t/tau_g) + D*np.exp(-t/tau_d)
...
def func_exp_decay10(t, A, tau_a, B, tau_b, G, tau_g, D, tau_d, E, tau_e):
    return (-0.316-A-B-G-D-E) + A*np.exp(-t/tau_a) + B*np.exp(-t/tau_b) + G*np.exp(-t/tau_g) + D*np.exp(-t/tau_d) + E*np.exp(-t/tau_e)
...
out_pref='out'
out_fn=out_pref+'_fittedCt_HCH.dat'
in_file_list = [out_pref+'_Ctint_HCH.dat']
bNoFast=True #False
nc=-1
...
for i in range(num_vecs):
    ...
    if fmod( num_pars, 2 ) == 1:
        ...
        print >> fp, '# Param %s: %g +- %g' % ('S2_0', -0.316-pars[0]-pars[2], 0.0)
