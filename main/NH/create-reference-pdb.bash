...
if [[ "$bMake" == "True" ]] ; then
    ...
    $gmxsele -s $tpr -n $ndx -select '"Solute"' #not group "Water_and_ions"' >& gmx.err || { cat gmx.err >&2 ; exit 1; }
else
    echo "= = $ndx exists, and group Solute has been found within it."
fi

# Create reference molecule with CoG at origin.
echo Solute | $editconf -f $tpr -o $opdb -n $ndx -nopbc -center 0 0 0  >& gmx.err || { cat gmx.err >&2 ; exit 1; }
