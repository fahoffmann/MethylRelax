...
Xseltxt1='(name CE and resname MET) or (name CG2 and resname ILE) or (name CD1 and resname ILE) or (name CG1 and resname VAL)'
Xseltxt2='(name CG2 and resname VAL) or (name CD1 and resname LEU)'
Xseltxt3='(name CD2 and resname LEU) or (name CG2 and resname THR) or (name CB and resname ALA)'
Cseltxt1='(name SD and resname MET) or (name CB and resname ILE) or (name CG1 and resname ILE) or (name CB and resname VAL)'
Cseltxt2='(name CB and resname VAL) or (name CG and resname LEU)'
Cseltxt3='(name CG and resname LEU) or (name CB and resname THR) or (name CA and resname ALA)'
...
if bDoCt:
        ...
        Ct_C, dCt_C = calculate_Ct_Palmer(vecCX)
        print_sxylist(out_pref+'_Ctext.dat', resXH, dt, np.stack( (Ct.T,dCt.T), axis=-1) )
        print_sxylist(out_pref+'_Ctext_C.dat', resCX, dt, np.stack( (Ct_C.T,dCt_C.T), axis=-1) )
        print "= = = Conducting Ct_internal using Palmer's approach."
        Ct, dCt = calculate_Ct_Palmer(vecXHfit)
        Ct_C, dCt_C = calculate_Ct_Palmer(vecCXfit)
        print_sxylist(out_pref+'_Ctint.dat', resXH, dt, np.stack( (Ct.T,dCt.T), axis=-1) )
        print_sxylist(out_pref+'_Ctint_C.dat', resCX, dt, np.stack( (Ct_C.T,dCt_C.T), axis=-1) )
        del Ct, dCt
        del Ct_C, dCt_C
sh = vecXHfit.shape
...
if bDoVecDistrib:
    ...
    if not bDoVecHist:
        ...
        if bBinary:
            ...
            np.savez_compressed(out_pref+'_vecHistogram_C.npz', names=resCX, dataType='LambertCylindrical', bHistogram=True, edges=edgesAll, axisLabels=['phi','cos(theta)'], data=hist_list)
        else:
            for i in range(nBonds):
