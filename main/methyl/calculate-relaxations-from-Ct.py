...
class relaxationModel:
    ...
    def get_relax_from_J(self, J, CSAvalue=None):
        ...
        # Note: since J is in the units of inverse time, data needs to be converted back to s^-1
        Rz = self.time_fact*( pref**2.0*3.0/16.0*( 0*J[0] + 1*J[1] + 4*J[2] ) )
        Ry = self.time_fact*( pref**2.0*1.0/32.0*( 9*J[0] +15*J[1] + 6*J[2] ) )
        R3 = self.time_fact*( pref**2.0*3.0/16.0*( 0*J[0] + 3*J[1] + 0*J[2] ) )
...
def _obtain_Jomega(RObj, nSites, S2, consts, taus, vecXH, weights=None):
    ...
    if RObj.rotdifModel.name == 'rigid_sphere':
        datablock=np.zeros((3,nSites), dtype=np.float32)
...
def _obtain_R1R2NOErho(RObj, nSites, S2, consts, taus, vecXH, weights=None, CSAvaluesArray=None):
    ...
    if RObj.rotdifModel.name == 'direct_transform':
        ...
        for i in range(nSites):
            J = J_direct_transform(RObj.omega, consts[i], taus[i])
        ...
        if len(vecXH.shape) > 2:
            ...
            for i in range(nSites):
                ...
                if weights is None:
                    ...
                    datablock[0,i]=weighted_average_stdev(tmpR1, weights[i])
                    datablock[1,i]=weighted_average_stdev(tmpR2, weights[i])
                    datablock[2,i]=weighted_average_stdev(tmpNOE, weights[i])
                    datablock[3,i]=weighted_average_stdev(tmprho, weights[i])
            return datablock
        else:
...
out_pref='out'
bHaveDy=False
in_Ct_fn=out_pref+'_fittedCt.dat'
in_Ct_fn2=out_pref+'_fittedCt_C.dat'
...
qrot_str=''
vecfn=None
distfn=out_pref+'_vecHistogram.npz'
...
if not bOptPars:
    if bJomega:
        ...
        datablock = _obtain_Jomega(relax_obj, num_vecs, S2_list, consts_list, taus_list, vecCX, weights=vecCXweights)
    else:
        ...
        datablock = _obtain_R1R2NOErho(relax_obj, num_vecs, S2_list, consts_list, taus_list, vecCX, weights=vecCXweights, CSAvaluesArray = CSAvaluesArray )
...
if bJomega:
    ...
    if not bHaveDy:
        print_xy(out_pref+'_Rz.dat',  sim_resid, datablock[0,:], header=optHeader)
        print_xy(out_pref+'_Ry.dat',  sim_resid, datablock[1,:], header=optHeader)
        print_xy(out_pref+'_R3z2-2.dat', sim_resid, datablock[2,:], header=optHeader)
        print_xy(out_pref+'_rho.dat', sim_resid, datablock[3,:])
    else:
        print_xydy(out_pref+'_Rz.dat',  sim_resid, datablock[0,:,0], datablock[0,:,1], header=optHeader)
        print_xydy(out_pref+'_Ry.dat',  sim_resid, datablock[1,:,0], datablock[1,:,1], header=optHeader)
        print_xydy(out_pref+'_R3z2-2.dat', sim_resid, datablock[2,:,0], datablock[2,:,1], header=optHeader)
